...
export default class ChartView extends React.Component {
  ...
  _requestCurveData() {
    ...
    let thatTradeCode = this.tradeCode;
    ...
    $.post(_url, {
      ...
      if (this.tradeCode != thatTradeCode) {
        // console.log("An expired request, ignore this result.");
        return;
      }
  ...
  _showCurveInChart(seriesData) {
    ...
    // console.log("===_showCurveInChart, current tradecode: " + this.tradeCode);
  ...
  update(data, timestamp) {
    ...
    if (data && data.length > 0) {
      for (var i = 0; i < data.length; i++) {
        let infoArr = data[i].split(",");
        if (infoArr && infoArr.length >= 3) {
          let name = infoArr[0];
          let timestamp = parseInt(infoArr[1]) * 1000;
          let price = parseFloat(infoArr[2]);
          if (isNaN(price)) {
            continue;
          }

          if (name == this.tradeCode) {
            socketData.push([timestamp, price]);
            // console.log("===update pt, current tradecode: " + this.tradeCode);
          }
        }
      }
    }
